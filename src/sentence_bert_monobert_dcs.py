...
from . import helper
from .search_models.code_search_manager import CodeSearchManager
from .mono_bert_dcs import MONOBERT_DCS
from .sentence_bert_dcs import SBERT_DCS

#Example of Multi-Stage architecture
...
# monoBert Rerank score
def re_rank_candidates(my_id, topN_ids, test_desc_str, test_code_str):
...
def topN_test(embedded_desc, embedded_tokens, test_desc_str, test_code_str):
    ...
    for rowid in range(len(embedded_tokens)):
        ...
        if not rowid in topN:
            ...
            results_extended[rowid] = re_rank_candidates(rowid, topN, test_desc_str, test_code_str)
        pbar.update(1)
    pbar.close()
...
def rephrasing_test(rephrased_descriptions_df, embedded_tokens, embedded_desc):
    ...
    for i, row in enumerate(rephrased_descriptions_df.iterrows()):
        ...
        if not idx in topN:
            ...
            original_rank = re_rank_candidates(idx, topN, test_desc_str, test_code_str)
        ...
        if not idx in topN:
            ...
            new_rank = re_rank_candidates(idx, topN, test_desc_str, test_code_str)
...
if __name__ == "__main__":
    ...
    monobert.load_weights(script_path + "/../weights/monobert_00001_dcs_plus_weights")

    sentence_bert.generate_bert_layer()
    sentence_bert.get_model()
    sentence_bert.generate_tokenizer()
    sentence_bert.load_weights(script_path + "/../weights/sbert_600k_00002_m20_dcs_weights")

    number_of_elements = 100
    test_tokens = helper.load_hdf5(data_path + "test.tokens.h5", 0, number_of_elements)
    test_desc = helper.load_hdf5(data_path + "test.desc.h5", 0, number_of_elements)  # 10000
    ...
    vocabulary_tokens = helper.load_pickle(data_path + "vocab.tokens.pkl")
    vocab_tokens = {y: x for x, y in vocabulary_tokens.items()}

    vocabulary_desc = helper.load_pickle(data_path + "vocab.desc.pkl")
    ...
    model_code = sentence_bert.model_code
    model_query = sentence_bert.model_query
